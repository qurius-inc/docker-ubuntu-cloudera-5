...
ADD scripts/start-base.sh /root/start-base.sh
RUN chmod +x /root/start-base.sh
...
RUN sed -i 's#"nohup \$AGENT_SCRIPT \$CMF_AGENT_ARGS" > \$AGENT_OUT 2>&1 </dev/null &#"nohup \$AGENT_SCRIPT \$CMF_AGENT_ARGS" > \$AGENT_OUT 2>\&1#g' /etc/init.d/cloudera-scm-agent
